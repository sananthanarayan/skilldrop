...
>
> **Status (2026-07): MVP implemented** — see [RFC-0002](../rfcs/0002-skilldrop-cli.md) and [`bin/skilldrop.js`](../../bin/skilldrop.js). Shipped: `list`, `info`, `packs`, `install` (skills / `--pack` / `--all`, `--with-related`, Claude user+project, Cursor and Kiro with wiring files, `--dest`), `update`, `outdated`, `uninstall`, per-destination `.skilldrop.json` ledger. **v0.2.0 adds third-party catalogs** ([RFC-0003](../rfcs/0003-third-party-catalogs.md)): `--from <path | git-url[#ref]>` on the read/install commands, provenance-aware update/outdated, a structural gate before any copy, and a `validate` command for catalog authors. Still design-only: `search`, `doctor`, IDE auto-detection, `@version` pinning, `--ide all`, registry index. **Naming correction:** `skilldrop` is taken on npm — the package is `skilldrop-cli`; the bin command stays `skilldrop`.
...
- **What's the "official" registry URL?** ~~v1 ships the registry inside the npm package itself, so there's no URL. v2 would need one (GitHub Pages on this repo? A `skilldrop.dev` site?). Decide at v2 time, not now.~~ **Answered 2026-07 ([RFC-0011](../rfcs/0011-catalogue-site.md)):** GitHub Pages on this repo — <https://sananthanarayan.github.io/skilldrop/>, with a machine-readable index at [`catalogue.json`](https://sananthanarayan.github.io/skilldrop/catalogue.json) generated from the same manifests. A `skilldrop.dev` domain remains a `CNAME` file away if it's ever wanted.
